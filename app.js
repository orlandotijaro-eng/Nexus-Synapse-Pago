...
const el = {
  ...
  btnAplicarNotaCredito: document.getElementById('btn-aplicar-nota-credito'),
...
async function cargarContabilidadFactura(factura) {
  ...
  if (!factura.cufe) {
    ...
    el.btnAplicarNotaCredito.hidden = true;
    return;
  }
  ...
  if (esNota) {
    el.contabFacturaCargando.hidden = true;
    el.contabFacturaVacia.hidden = false;
    el.contabFacturaVaciaTexto.textContent = 'Este documento es una nota crédito/débito, no se contabiliza como una factura normal.';
    el.btnContabilizarAhora.hidden = true;
    el.btnAplicarNotaCredito.hidden = false;
    return;
  }
...
function abrirModalAplicarNC() {
  const nc = facturaActiva();
  if (!nc) return;
  state.ncOrigen = nc;
  el.aplicarNcOrigen.textContent = `Nota: ${nc.nroFactura || ''} · ${nc.vendedorNombre || ''} · ${formatoMoneda(nc.total)}`;
...
function renderListaAplicarNC(filtro) {
  ...
  const candidatas = state.facturas
    .filter((f) => (f.saldo || 0) > 0 && !(f.documento || '').toUpperCase().includes('NOTA') && f.cufe !== (nc && nc.cufe))
    .filter((f) => !texto || [f.vendedorNombre, f.nroFactura, String(f.vendedorNit || '')].join(' ').toLowerCase().includes(texto))
    .slice(0, 60);
  if (!candidatas.length) {
    el.aplicarNcLista.innerHTML = '<p class="pago-base-hint">No hay facturas por pagar que coincidan.</p>';
    return;
  }
...
el.btnAplicarNotaCredito.addEventListener('click', abrirModalAplicarNC);
