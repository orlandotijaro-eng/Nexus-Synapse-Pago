...
async function subirUnZip(archivo, url) {
...
async function procesarLoteZip(archivos, url) {
  const errores = [];
  let exitosos = 0;
  ...
  for (let i = 0; i < archivos.length; i++) {
    ...
    const resultado = await subirUnZip(archivo, url);
    if (resultado.ok) {
      exitosos++;
    } else {
  ...
  if (errores.length === 0) {
    mostrarMensajeZip(`Listo: ${exitosos} de ${archivos.length} documentos procesados correctamente.`, 'ok');
  } else {
    mostrarMensajeZip(`${exitosos} de ${archivos.length} procesados. Con problema: ${errores.join(' · ')}`, 'error');
  }
